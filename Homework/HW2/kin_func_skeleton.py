...
def rotation_3d(omega, theta):
    ...
    second_term = (skew_3d(omega) / omega_norm) * np.sin(omega_norm * theta)

    third_term = ((np.linalg.matrix_power(skew_3d(omega), 2)) / (omega_norm**2)) * (1 - np.cos(omega_norm * theta))

    rot = np.identity(3) + second_term + third_term
...
def hat_3d(xi):
    ...
    xi_hat = np.array([ [0, -omega_z, omega_y, v_x], [omega_z, 0, -omega_x, v_y], [-omega_y, omega_x, 0, v_z], [0, 0, 0, 0] ])
...
def homog_3d(xi, theta):
    ...
    # YOUR CODE HERE
    v = np.array([[xi[0], xi[1], xi[2]]]).T

    omega = np.array([[xi[3], xi[4], xi[5]]]).T

    matrix_exp = rotation_3d(omega, theta)

    omega_norm = np.linalg.norm(omega)

    expanded_term = (1/(omega_norm**2)) * ((np.matmul((np.identity(3) - matrix_exp) , (np.matmul(skew_3d(omega), v)))) + (np.matmul((np.outer(omega, omega)) , v * theta)))

    g = np.array([ [matrix_exp[0,0][0], matrix_exp[0,1][0], matrix_exp[0,2][0], expanded_term[0][0][0]], [matrix_exp[1,0][0], matrix_exp[1,1][0], matrix_exp[1,2][0], expanded_term[1][0][0]], [matrix_exp[2,0][0], matrix_exp[2,1][0], matrix_exp[2,2][0], expanded_term[2][0][0]], [0, 0, 0, 1] ])
...
def prod_exp(xi, theta):
    ...
    # YOUR CODE HERE
    g = np.matmul(np.matmul(homog_3d(xi[:,0], theta[0]), homog_3d(xi[:,1], theta[1])), homog_3d(xi[:,2], theta[2])) 
